...
import { renderLppPopup, renderIjppPopup, renderTrainPopup, createBusStopPopup, createTrainStopPopup } from "./popups"
...
function attachPopup(map, layerId, formatter, afterOpen) {
  map.on("click", layerId, (event) => {
    ...
    const popup = new maplibregl.Popup({ closeButton: false }).setLngLat(event.lngLat)
...
export function configureBusStopPopup({ map, onSelectStop }) {
  map.on("click", "busStops-points", (event) => {
    ...
    const popupContent = createBusStopPopup(props, [lat, lng], onSelectStop)

    new maplibregl.Popup({ closeButton: false }).setLngLat([lng, lat]).setDOMContent(popupContent).addTo(map)
  })
}
...
export function configureTrainStopPopup({ map, onSelectStop }) {
  map.on("click", "trainStops-points", (event) => {
    ...
    const props = feature.properties || {}
    const [lng, lat] = feature.geometry.coordinates
    const popupContent = createTrainStopPopup(props, [lat, lng], onSelectStop)
...
export function configureTrainPopup({ map, onSelectVehicle, onNavigateRoute }) {
  attachPopup(map, "trainPositions-points", renderTrainPopup, (popup, properties) => {
    if (!properties) return
    const container = popup.getElement()
    if (!container) return
    const button = container.querySelector('[data-role="view-sz-route"]')
    if (!button) return
      ...
      (event) => {
        ...
        if (typeof from === "string") {
          try {
            ...
            console.warn("Ne morem razvozljati podatkov 'from':", error)
            from = null
          }
        }
        ...
        if (typeof to === "string") {
          try {
            ...
            console.warn("Ne morem razvozljati podatkov 'to':", error)
            to = null
          }
        }
        ...
        onSelectVehicle({
          ...
          realTime: properties.realTime === true || properties.realTime === "true",
          from,
          to,
        })
        onNavigateRoute()
...
export function configureBusPopup({ map, onSelectVehicle, onNavigateRoute }) {
  attachPopup(
    ...
    (properties) => {
      ...
      if (properties.sourceType === "lpp") return renderLppPopup(properties)
      if (properties.sourceType === "ijpp") return renderIjppPopup(properties)
      return `<div style="min-width:180px">Ni podatkov</div>`
    },
    (popup, properties) => {
      if (!properties) return
            ...
            (event) => {
              ...
              if (Array.isArray(rawStops)) {
                ...
                try {
                  ...
                  console.warn("Neveljaven format postaj:", err)
                }
              }
              onSelectVehicle({
      ...
      if (properties.sourceType === "lpp") {
        const button = container.querySelector('[data-role="view-lpp-route"]')
        if (button) {
          button.addEventListener(
            "click",
            (event) => {

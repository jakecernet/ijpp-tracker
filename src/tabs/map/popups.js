...
export function createRow(label, value) {
  ...
  return (
    ...
    `<span style="font-weight:600; text-align:right">${escapeHTML(String(value))}</span>` +
    `</div>`
  )
}
...
export function renderLppPopup(properties) {
  ...
  return (
    ...
    (title ? `<div style="font-weight:500; font-size:16px; margin-bottom:8px">${title}</div>` : "") +
    rows +
...
export function renderIjppPopup(properties) {
  const heading = properties.lineName || properties.title || properties.routeId || "Vozilo"
  const operator = createRow("Prevoznik", properties.operator)
  const stop = createRow(
    properties.stopStatus === "STOPPED_AT" ? "Na postaji: " : "Naslednja postaja: ",
    properties.stop,
  )

  return (
    `<div style="min-width:240px">` +
    `<div style="font-weight:700; font-size:16px; margin-bottom:8px">${escapeHTML(String(heading))}</div>` +
    operator +
    stop +
...
export function renderTrainPopup(properties) {
  ...
  return (
    ...
    (number ? `<div style="font-weight:600; font-size:16px; margin-bottom:4px">${escapeHTML(number)}</div>` : "") +
    (departure
...
export function createBusStopPopup({ name, id, ref_id, gtfs_id }, coordinates, onSelect) {
...
export function createTrainStopPopup({ name, stopId, id }, coordinates, onSelect) {
  const wrapper = document.createElement("div")
  const title = document.createElement("h3")
  title.textContent = name || ""
  wrapper.appendChild(title)

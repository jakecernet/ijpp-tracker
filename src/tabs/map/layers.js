import { CLUSTER_CONFIG, ICON_SIZE_BY_LAYER, ICON_ANCHOR_BY_LAYER, BRAND_COLORS, HALO_RADIUS } from "./config"
...
function ensureClusterLayers(map, id, color) {
  if (!map.getLayer(`${id}-clusters`)) {
    map.addLayer({
      ...
      paint: {
        "circle-color": color,
        "circle-radius": ["step", ["get", "point_count"], 10, 20, 13, 50, 17],
        "circle-opacity": 0.8,
      },
    })
  }
...
export function setupSourcesAndLayers(map, dataBySource) {
  Object.entries(CLUSTER_CONFIG).forEach(([id, config]) => {
    ensureSource(map, id, dataBySource[id], true, config.radius, config.maxZoom)
    ensureClusterLayers(map, id, config.color)
    ensurePointLayer(map, id, ICON_SIZE_BY_LAYER[id], ICON_ANCHOR_BY_LAYER[id])
...
export function setPrefixVisible(map, prefix, visible) {
  ...
  const layers = [`${prefix}-points`, `${prefix}-clusters`, `${prefix}-cluster-count`, `${prefix}-halo`]
